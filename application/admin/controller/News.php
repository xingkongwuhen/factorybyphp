<?php
  ...
  class News extends Base
  {
      public function index()
      {
        ...
        $vlist = $this->db->table('news')->where($where)->order('orderid asc')->pages($pagesize);

        foreach($vlist['lists'] as $key => $value )
        {  //循环输出各个影片的类型
          $type =$this->db->table('newstype')->field('title')->where(array('id'=>$value['type']))->info();
      ...
      {
        $mid = (int)input('get.mid');
        $typelist = $this->db->table('newstype')->where(array('status'=>0,'is_del'=>0,'mid'=>$mid))->lists();
        if($typelist)
        {
          $this->assign('mid',$mid);
          ...
          echo '<script>
            alert("请先添加分类！");
            window.location.href="/public/admin/News/type";
          </script>';
      ...
      public function changetj()
      {
        ...
        if($status['tj'] == 0)
        {
          $res = $this->db->table('news')->where(array('id'=>$id))->update(['tj'=>1]);
          if($res){
            $status = 1;
            $msg = "推荐成功";
          }else{
          ...
          $res = $this->db->table('news')->where(array('id'=>$id))->update(['tj'=>0]);
          if($res){
            $status = 1;
            $msg = "取消推荐成功";
          }else{
      ...
      {
        $mid = (int)input('get.mid');
        $vlist = $this->db->table('news')->where(array('is_del'=>1,'mid'=>$mid))->lists();
        $this->assign("list",$vlist);
        $this->assign('mid',$mid);
        return $this->fetch();
      }
      ...
      public function undelet()
      {
        ...
        if($id > 0){
          ...
          $res = $this->db->table('news')->where(array('is_del'=>1,'mid'=>$mid))->update(['is_del'=>0,'deltime'=>null]);
        }
        if(!$res)
        {
          $status = 0;
      ...
      public function delet()
      {
        ...
        if($id > 0){
          ...
          $res = $this->db->table('news')->where(array('is_del'=>1,'mid'=>$mid))->del();
        }
        if(!$res)
        {
          $status = 0;
      ...
      {
        $mid = (int)input('get.mid');
        $typelist = $this->db->table('newstype')->where(array('is_del'=>0,'status'=>0,'mid'=>$mid))->lists();
        if($typelist)
        {
          $typelist = $typelist;
        }else{
      ...
      {
        $mid = (int)input('get.mid');
        $vlist = $this->db->table('newstype')->where(array('is_del'=>1,'mid'=>$mid))->lists();
        $this->assign("list",$vlist);
        $this->assign('mid',$mid);
        return $this->fetch();
      }
      ...
      public function typeundelet()
      {
        ...
        if($id > 0){
          ...
          $res = $this->db->table('newstype')->where(array('is_del'=>1,'mid'=>$mid))->update(['is_del'=>0,'deltime'=>null]);
        }
        if(!$res)
        {
          $status = 0;
      ...
      public function typedelet()
      {
        ...
        if($id > 0){
          ...
          $res = $this->db->table('newstype')->where(array('is_del'=>1,'mid'=>$mid))->del();
        }
        if(!$res)
        {
          $status = 0;
